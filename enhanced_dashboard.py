...
def get_current_growth_stage():
    ...
    for stage, info in TOBACCO_CALENDAR.items():
        stage_date = datetime(current_date.year, info["month"], info["day"])
        if stage_date < current_date and info["month"] > 6:  # Handle year transition
            stage_date = datetime(current_date.year - 1, info["month"], info["day"])
...
def create_risk_summary_cards(forecast_alerts):
    ...
    next_24h_risks = len([a for a in forecast_alerts if a["datetime"] <= datetime.now() + timedelta(hours=24)])
    high_severe_risks = len([a for a in forecast_alerts if a["risk_level"] >= 3])
    
    # Find next risk
    next_risk = min(forecast_alerts, key=lambda x: x["datetime"])
    hours_to_next = (next_risk["datetime"] - datetime.now()).total_seconds() / 3600
...
def main():
    ...
    if forecast_alerts:
        ...
        next_24h_alerts = [a for a in forecast_alerts if a["datetime"] <= datetime.now() + timedelta(hours=24)]
        st.markdown(f"""
    ...
    with tab4:
        ...
        **📊 Trend Analysis Information:**
        - Charts show **forecasted values** (model predictions) for the next 24 hours
        - Current real-time temperature: **{:.1f}°C** (sensor data)
        - Risk calculations include growth stage multipliers
        - Risk alerts are highlighted on the timeline
        """.format(current_temp))
        ...
        fig_temp = px.line(trend_df, x="Time", y=["Forecasted Temperature", "Humidity"], 
                          title=f"Forecasted Temperature & Humidity Trend - {stage_name} (Next 24 Hours)")
        ...
        fig_temp.update_layout(
            annotations=[
                dict(
                    ...
                    text="📊 Based on weather model predictions (not real-time sensor data)",
        ...
        fig_risk = px.line(trend_df, x="Time", y=["Dust Risk", "Hail Risk", "Rain Risk"],
                          title=f"Stage-Adjusted Risk Trends - {stage_name} (Multiplier: {stage_multiplier}x)")
        
        # Add risk threshold line
        fig_risk.add_hline(y=2, line_dash="dot", line_color="orange", 
                          annotation_text="Risk Alert Threshold", annotation_position="right")
        ...
        fig_risk.update_layout(
            annotations=[
                dict(
                    x=0.5, y=1.05, xref='paper', yref='paper',
                    text="⚠️ Risk calculations based on forecasted weather conditions",
                    showarrow=False, font=dict(size=12, color="gray")
                )
            ]
        )
        ...
        st.markdown("### 📊 Risk Timeline Summary")
        
        col1, col2 = st.columns(2)
        with col1:
            # Risk distribution chart
            risk_counts = trend_df.groupby('Max Risk').size().reset_index(name='Hours')
            risk_counts['Risk Level'] = risk_counts['Max Risk'].apply(get_risk_intensity_label)
            
            fig_dist = px.bar(risk_counts, x='Risk Level', y='Hours', 
                            title="Risk Level Distribution (Next 24h)",
                            color='Max Risk', color_continuous_scale='RdYlGn_r')
        ...
        with col2:
            ...
            if forecast_alerts:
                ...
                for alert in forecast_alerts[:10]:  # Show next 10 alerts
                    hours_from_now = (alert["datetime"] - datetime.now()).total_seconds() / 3600
                ...
                fig_timeline = px.scatter(alert_df, x="Hours from Now", y="Risk Level", 
                                        ...
                                        title="Upcoming Risk Alerts Timeline",
                                        hover_data=["Time"])
                fig_timeline.update_layout(yaxis=dict(range=[0, 5]))
                st.plotly_chart(fig_timeline, use_container_width=True)
            else:
                st.success("✅ No risk alerts in the forecast period")
        
        # Enhanced risk trend summary
        st.markdown(f"""
        **📋 Trend Analysis Notes:**
        - Risk levels shown include {stage_name} vulnerability multiplier ({stage_multiplier}x)
        - Higher multipliers during critical growth stages reflect increased sensitivity to weather conditions
        - **Temperature trends** are based on forecast models, not real-time sensor readings
        - **Current vs Forecast:** Real-time temperature is {current_temp:.1f}°C, next hour forecast is {forecast_temp:.1f}°C (±{abs(current_temp - forecast_temp):.1f}°C difference)
        - **Risk Alerts:** {len(forecast_alerts)} total alerts detected in forecast period
        - **Next 24h Alerts:** {len([a for a in forecast_alerts if a["datetime"] <= datetime.now() + timedelta(hours=24)])} alerts in next 24 hours
        """)
        ...
        if not forecast_alerts:
            ...
            **Monitoring Status:**
            ...
            if alert_view == "Next 24h Only":
                display_alerts = [a for a in display_alerts if a["datetime"] <= datetime.now() + timedelta(hours=24)]
            ...
            with col2:
                ...
                if display_alerts:
                    ...
                    st.download_button(
                        ...
                        data=csv_string,
                        file_name=f"weather_alerts_{selected_region}_{datetime.now().strftime('%Y%m%d')}.csv",
                        mime="text/csv"
                    )
